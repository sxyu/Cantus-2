...
Namespace UI
    Module MainModule
        ...
        <STAThread>
        Public Sub Main()
            ' Check if settings is corrupted. If so, prompt the user.
            Try
                My.Settings.Reload()
            Catch ex As ConfigurationException
                Dim filename As String = (CType(ex.InnerException, ConfigurationException)).Filename
                MessageBox.Show("Cantus has detected that your" +
                          " user settings file has become corrupted. " +
                          "This may be due to a crash or improper exiting" +
                          " of the program. Cantus must now reset your " +
                          "user settings in order to continue.\n\nIf this persists, please submit" +
                          " a ticket to http://github.com/sxyu/Cantus-GUI", "Settings Corrupted",
